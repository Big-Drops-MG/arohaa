...
type EventTrackingKpiRowProps = {
  ...
  activeKpiId: EventTrackingMetricId
  onKpiSelect: (id: EventTrackingMetricId) => void
}

function gridClassName(count: number): string {
  if (count <= 4) {
    return "grid grid-cols-2 gap-3 md:grid-cols-2 xl:grid-cols-4"
  }
  return "grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5"
}

export function EventTrackingKpiRow({
  kpis,
  activeKpiId,
  onKpiSelect,
}: EventTrackingKpiRowProps) {
  ...
  return (
    <motion.div
      ...
      animate="visible"
      className={gridClassName(kpis.length)}
    >
      {kpis.map((kpi) => {
        ...
        return (
          ...
          >
            {active ? (
              <motion.div
                layoutId="event-tracking-kpi-active"
