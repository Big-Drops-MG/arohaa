...
type OverviewHeaderProps = {
  title?: string
  /** Override the default subtitle under the title. Pass `null` to hide it. */
...
export function OverviewHeader({
  ...
  const showDateRange =
    Boolean(dateRangeOptions) &&
    dateRangeId != null &&
    typeof onDateRangeChange === "function" &&
    typeof onCustomRangeChange === "function"

  const selectedLabel = useMemo(() => {
    if (!dateRangeId) return ""
  ...
  const presetOptions = (dateRangeOptions ?? []).filter(
    (opt) => opt.id !== "custom"
  )
  ...
  const resolvedSubtitle =
    subtitle === undefined ? "Landing page performance at a glance" : subtitle
  const showControls = Boolean(actions) || Boolean(projectId) || showDateRange
