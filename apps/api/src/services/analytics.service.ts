...
function overviewHourlyMetricsQuery(formType: LandingFormType): string {
...
function overviewDailyMetricsQuery(): string {
  return `
    SELECT
      day AS bucket,
...
function overviewHourlyBounceQuery(): string {
  return `
    SELECT
...
function overviewDailyBounceQuery(): string {
  return `
    SELECT
...
export async function getAnalyticsOverview(
  ...
  const submitEvent = submissionEventName(formType)
  const cacheKey = `analytics:overview:v2:${workspaceId}:${formType}`
  try {
  ...
  const [
    ...
    engagedRes,
      ...
      ch.query({
        format: 'JSON',
        query_params: p,
        query: overviewHourlyMetricsQuery(formType),
      }),

      ch.query({
        format: 'JSON',
        query_params: p,
        query: overviewDailyMetricsQuery(),
      }),

      ch.query({
        format: 'JSON',
        query_params: p,
        query: overviewHourlyBounceQuery(),
      }),

      ch.query({
        format: 'JSON',
        query_params: p,
        query: overviewDailyBounceQuery(),
      }),
